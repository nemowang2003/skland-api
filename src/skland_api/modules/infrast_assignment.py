...
def cost(roster: FacilityRoster, presence: FacilityPresence) -> tuple[int, int]:
    expected = set(roster)
    actual = set(operator.name for operator in presence)
...
def align_facilities(
    ...
    # perm[k] 就是 expected[k] 对应 actual[perm[k]]
    perm_it = permutations(range(len(rosters)))
    best_perm = next(perm_it)
    best_cost = sum(
        (cost(rosters[k], presences[v]) for k, v in enumerate(best_perm)),
        start=(0, 0),
    )
    for perm in perm_it:
        total = sum(
            (cost(rosters[k], presences[v]) for k, v in enumerate(perm)),
            start=(0, 0),
        )
        if total < best_cost:
            best_cost = total
    ...
    # return zip(rosters, map(presences.__getitem__, best_perm))
