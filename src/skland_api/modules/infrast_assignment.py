    ...
    def new(cls, infrast_presence: InfrastPresence, active_roster: InfrastRoster) -> Self:
        return cls(
            ...
            powers=[
                ...
                for roster, presence in zip(active_roster.powers, infrast_presence.powers)
            ],
            tradings=[
                FacilityAudit.from_facility(roster, presence)
                for roster, presence in zip(active_roster.tradings, infrast_presence.tradings)
            ],
            manufactures=[
                FacilityAudit.from_facility(roster, presence)
                for roster, presence in zip(
            ...
            dormitories=[
                FacilityAudit.from_facility(roster, presence)
                for roster, presence in zip(active_roster.dormitories, infrast_presence.dormitories)
            ],
        )
...
def main(character_info: CharacterInfo, config: dict | None) -> InfrastAssignmentReport:
    ...
    if len(active_rosters) == 0:
        ...
        audit = InfrastAudit.new(infrast_presence, InfrastRoster.from_maa_roster(active_rosters[0]))
